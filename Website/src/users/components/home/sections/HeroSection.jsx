    ...
    useEffect(() => {
        const fetchServices = async () => {
            try {
                ...
                const response = await fetch(`${API_BASE_URL}/api/services`, { headers: getAuthHeaders() });

                // Check if response is JSON before parsing
                const contentType = response.headers.get('content-type');
                if (!contentType || !contentType.includes('application/json')) {
                    console.warn('API endpoint /api/services not available or returned non-JSON response');
                    setServices([]);
                    return;
                }
                ...
                if (response.ok && data.success && Array.isArray(data.data)) {
                    ...
                    // Silently fail - search is optional functionality
                    console.warn("Failed to load services for search:", data.message);
                    setServices([]);
                }
            } catch (error) {
                // Silently fail - search is optional functionality
                console.warn("Unable to load services for search:", error.message);
                setServices([]);
            } finally {
            ...
            <div className="relative mx-auto max-w-7xl px-4 py-5 sm:py-7 lg:py-10 mt-20 sm:mt-24 lg:mt-24">
                <div className="flex flex-col items-center gap-14 lg:flex-row lg:items-center lg:gap-20">
                    ...
                    >
                        ...
                        {/* Dual CTAs - Book Service + Become Professional */}
                        <motion.div
                            variants={staggerItem}
                        ...
                        >
                            ...
                            <button
                                onClick={() => setShowProfessionalModal && setShowProfessionalModal(true)}
                                className="px-8 py-4 rounded-xl font-semibold text-lg transition-all duration-300 flex items-center justify-center gap-2 cursor-pointer bg-card text-foreground border-2 border-border hover:border-primary"
                            >
