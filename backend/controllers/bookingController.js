...
exports.createBooking = async (req, res) => {
  try {
    const { service, issueType, description, scheduledTime, location } =
      req.body;
    ...
    pushNotificationController
      .sendToMatchingWorkers(service, district, {
...
exports.getProfessionalBookings = async (req, res) => {
  try {
    const professionalId = req.user?.professionalId || req.user?.userId;
  ...
  try {
    ...
    if (professionalId && status === "assigned") {
      ...
      try {
        ...
        const professionalRecord = await Professional.findOne({
          phone: professional.phone,
        });
        if (professionalRecord) {
          professionalRecord.totalJobs =
            (professionalRecord.totalJobs || 0) + 1;
          await professionalRecord.save();
          console.log(
            `Incremented totalJobs for professional ${professionalRecord.name} to ${professionalRecord.totalJobs}`
          );
        } else {
          console.warn(
            `Professional record not found for user ${professionalId}`
          );
        }
      } catch (err) {
...
exports.getProfessionalByPhone = async (req, res) => {
  try {
    ...
    const professional = await Professional.findOne({ phone }).select(
      "service district phone name email"
    );
  ...
  try {
    ...
    if (!bookingId || !rating || !comment || payment === undefined) {
      return res.status(400).json({
        success: false,
        message:
          "Please provide all required fields (bookingId, rating, comment, payment)",
      });
    }
